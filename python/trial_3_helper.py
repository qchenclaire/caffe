...
sys.path.append('/mnt/4T-HD/qchen/RenderForCNN_2')
sys.path.append(os.path.join('/mnt/4T-HD/qchen/RenderForCNN_2', 'render_pipeline'))
cache_dir = '/mnt/4T-HD/qchen/cache'
log_dir = 'experiment/trial_3'
...
weights = np.ones(12 * 12 * 36)
weights[np.arange(2 * 12 * 36, 3 * 12 * 36)] = 0
weights[np.arange(3 * 12 * 36, 4 * 12 * 36)] = 0.4
g_dist = g_dist * weights
...
def sample_views(dist, size, dirname, epoch):
    ...
    for i in tmp.keys():
        file = os.path.join('statistics/dist_3',str(i)+'.npy')
...
# start_time = time.time()
# sample_views(g_dist, 5000, os.path.join('experiment/trial_3', str(3)), 3)
