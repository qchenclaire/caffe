...
class Data(caffe.Layer):
    ...
    def setup(self, bottom, top):
        ...
        self.batch_size_real = int(self.batch_size * 0.2)
    ...
    def forward(self, bottom, top):
        ...
        if sd.mode == 'val':
            ...
            self.val_idx = (self.val_idx + self.batch_size) % 11520
        ...
        if sd.mode == 'train':
            ...
            while (not os.path.exists(label_hard)):
                print 'waiting for label at train_iter %d' %(sd.train_iter)
                time.sleep(1)
            while (not os.path.exists(data_hard)):
                print 'waiting for data at train_iter %d' %(sd.train_iter)
                time.sleep(1)
            self.data_hard= np.load(data_hard)
            self.label_hard = np.load(label_hard)
            #print ('self.label_hard', self.label_hard.shape)
...
class Accuracy_3D(caffe.Layer):
    ...
    def forward(self, bottom, top):
        ...
        if sd.mode == 'val':
            ...
            if ((self.val_iter % 45) == 0):
                print 'validation at epoch %d' %((self.val_iter -1)/45)
                dirname = os.path.join('experiment/trial_4', str(sd.val_epoch))
                if not os.path.exists(dirname):
                    os.mkdir(dirname)
                updated_dist = update_dist(self.acc, self.val_count)
                tmp = os.path.join(dirname, 'updated_dist.npy')
                np.save(tmp, updated_dist)
                os.system('python python/sample_hard_4.py %s %d %s %d &' %(tmp, sd.render_size, dirname, sd.val_epoch))
...
class Accuracy(caffe.Layer):
    ...
    def setup(self, bottom, top):
        ...
        self.val_count = np.load('statistics/val_count.npy')
        self.vnum_test = 24
        self.azimuth_interval = np.arange(0, 360+1, (360/self.vnum_test)) - float(360/self.vnum_test)
        self.azimuth_interval[0] = 0
        self.acc = np.zeros(12 * self.vnum_test)
        self.true_positive = np.zeros(12 * self.vnum_test)
        self.false_positive = np.zeros(12 * self.vnum_test)
        self.false_negative = np.zeros(12 * self.vnum_test)
        self.val_iter = 0
    ...
    def forward(self, bottom, top):
        ...
        if sd.mode == 'val':
            ...
            for i in range(bottom[0].num):
                ...
                out_az = tmp_az[np.arange(az360[i]+16, 360+az360[i] -15)%360]
                pred_az = np.argmax(tmp_az)
                pred_bin = find_interval(pred_az, self.azimuth_interval)
                gt_bin = find_interval(az360[i], self.azimuth_interval)

                #print 'det',det
                if (pred_bin == pred_bin):
                    self.true_positive[cls_idx[i]*self.vnum_test + gt_bin] += 1
                    self.acc[cls_idx[i]*self.vnum_test + gt_bin] += 1
                else:
                    self.false_positive[cls_idx[i]*self.vnum_test + pred_bin] += 1
                    self.false_negative[cls_idx[i]*self.vnum_test + gt_bin]+= 1

                accuracy += (pred_bin == pred_bin)
