...
sys.path.append(os.path.join('/mnt/4T-HD/qchen/RenderForCNN', 'render_pipeline'))
cache_dir = '/mnt/4T-HD/qchen/cache'
log_dir = 'experiment/trial_2'
...
def sample_views(dist, size, dirname, epoch):
    ...
    for i in tmp.keys():
        file = os.path.join('statistics/dist',str(i)+'.npy')
...
# start_time = time.time()
# sample_views(g_dist, 30000, os.path.join('experiment/trial_2', str(0)), 0)

# print time.time()-start_time, 's'

def move2cache(epoch):
    path1 = []
    path1.append(os.path.join(log_dir, str(epoch)))
    path1.append(os.path.join(g_syn_images_folder, str(epoch)))
    path1.append(os.path.join(g_syn_images_bkg_overlaid_folder, str(epoch)))
    path1.append(os.path.join(g_syn_images_lmdb_folder,str(epoch)))
    path2 = []
    path2.append(os.path.join(cache_dir, 'logs'))
    path2.append(os.path.join(cache_dir, 'syn_images'))
    path2.append(os.path.join(cache_dir, 'syn_overlaid'))
    path2.append(os.path.join(cache_dir, 'syn_lmdbs'))

    for i in range(4):
        if not os.path.exists(path2[i]):
            os.mkdir(path2[i])
            os.system('mv %s %s &' %(path1[i], path2[i]))
