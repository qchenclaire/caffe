...
sys.path.append('/mnt/4T-HD/qchen/RenderForCNN')
...
shape_view_params = [[float(x) for x in line.strip().split(' ')] for line in open(shape_view_params_file).readlines()]
...
cls = int(shape_view_params[0][1])
ind = int(shape_view_params[0][0])
shape_synset = g_shape_synsets[cls]
model = ind - model_class[cls]
shape_file = pickle.load( open(os.path.join(g_shapenet_root_folder,'tmp',g_shape_names[cls]+'.p'), "rb" ) )
shape_file = os.path.join(g_shapenet_root_folder,shape_file[model])
start_time = time.time()
bpy.ops.import_scene.obj(filepath=shape_file)
sys.stdout.write('--- importing takes %d seconds ---' %(time.time()-start_time))
sys.stdout.flush()
start_time = time.time()
syn_images_folder = sys.argv[-1]
if (not os.path.exists(syn_images_folder)):
    sys.stdout.write('syn folder not exists')
    sys.stdout.flush()
    os.mkdir(syn_images_folder)
for i in range(len(shape_view_params)):
...
# YOUR CODE START HERE
    view_params = shape_view_params[i][2:]
    azimuth_deg = view_params[0]
    elevation_deg = view_params[1]
    theta_deg = view_params[2]
    rho = view_params[3]

	# set point lights
    for j in range(random.randint(light_num_lowbound,light_num_highbound)):
...
#syn_image_file = './%s_a%03d_e%03d_t%03d.png' % (ind, round(azimuth_deg), round(elevation_deg), round(theta_deg))
    syn_image_file = './m%d_c%d_a%03d_e%03d_t%03d_d%03d.png' % (ind, cls, round(azimuth_deg), round(elevation_deg), round(theta_deg), round(rho))
    bpy.data.scenes['Scene'].render.filepath = os.path.join(syn_images_folder, syn_image_file)
    bpy.ops.render.render( write_still=True )
    bpy.data.objects['Camera'].select = False
bpy.ops.object.delete()
sys.stdout.write('--- rendering takes %d seconds ---' %(time.time()-start_time))
