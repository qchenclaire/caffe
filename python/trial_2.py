...
class Data(caffe.Layer):
    ...
    def setup(self, bottom, top):
        ...
        self.batch_size_easy = self.batch_size - self.batch_size_hard
    ...
    def forward(self, bottom, top):
        ...
        if sd.mode == 'train':
            ...
            self.data = np.row_stack((self.data_easy, self.data_hard))
            self.label = np.row_stack((self.label_easy, self.label_hard))
    ...
    def forward(self, bottom, top):
        if sd.mode == 'val':
            ...
            if ((self.val_iter % 61) == 0):
                sys.stdout.write('validation at epoch %d' %((self.val_iter -1)/61))
                sys.stdout.flush()
                dirname = os.path.join('experiment/trial_2', str(sd.val_epoch))
                if not os.path.exists(dirname):
                    os.mkdir(dirname)
                updated_dist = update_dist(self.acc, self.val_count)
                tmp = os.path.join(dirname, 'updated_dist.npy')
                np.save(tmp, updated_dist)
                os.system('python python/sample_hard.py %s %d %s %d &' %(tmp, sd.render_size, dirname, sd.val_epoch))
                ...
                sd.mode = 'train'
                sd.val_epoch += 1
                print 'sample hard in the backgroud'
                sys.stdout.flush()
